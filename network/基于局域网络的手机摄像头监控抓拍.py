...
if filepath:
    filepath=filepath+'/'
##选择摄像头
videoLeftUp = cv2.VideoCapture("http://admin:admin@192.168.2.104:8081/")
videoRightUp = cv2.VideoCapture("http://admin:admin@192.168.2.177:8081/")
width = (int(videoLeftUp.get(cv2.CAP_PROP_FRAME_WIDTH)))
height = (int(videoLeftUp.get(cv2.CAP_PROP_FRAME_HEIGHT)))
num=0

if videoLeftUp.isOpened() and videoRightUp.isOpened():
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    # 第三个参数则是镜头快慢的，10为正常，小于10为慢镜头
    out = cv2.VideoWriter(filepath + '监控.avi', fourcc, 10, (width*2, height))
    out1 = cv2.VideoWriter(filepath + '监控1.avi', fourcc, 10, (width, height))
    out2 = cv2.VideoWriter(filepath + '监控2.avi', fourcc, 10, (width, height))
    while 1:
        ...
        if retLeftUp and retRightUp:
            ...
            '''
            a = out.write(frameUp)
            b=out1.write(frameLeftUp)
            c=out2.write(frameRightUp)
            ...
            if key == 27 or key == ord('q'):
                ...
                cv2.imencode('.png', frameUp)[1].tofile(filepath + filename)
            elif key == ord('1'):
                # 保存一张图像
                num = num + 1
                filename = "抓拍%s.png" % num
                ...
                filename = "抓拍%s.png" % num
                # cv2.imwrite(filepath+filename, frame)
                cv2.imencode('.png', frameRightUp)[1].tofile(filepath + filename)
        else:
